-- Backend: tmux
-- Sends text via tmux load-buffer | paste-buffer (vim-slime approach).
...
local function list_panes()
    local handle = io.popen("tmux list-panes -a -F '#{pane_id} #{session_name}:#{window_index}.#{pane_index} #{window_name}#{?window_active, (active),}' 2>/dev/null")
...
function M.open()
    ...
    if pane_id == '' then
        vim.api.nvim_echo({ { 'kitty-repl: tmux returned no pane ID — try window_kind="attach"', 'ErrorMsg' } }, false, {})
        return
    end
...
function M.send(text)
    ...
    -- (vim-slime bracketed_paste logic)
    ...
    -- Chunk and send via load-buffer stdin + paste-buffer (vim-slime approach)
